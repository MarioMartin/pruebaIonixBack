package cl.ionix.ms.prueba.config;
...
public class SpringfoxConfig {
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).select()
				...
				.build().apiInfo(getApiInfo()).enable(isEnabled).useDefaultResponseMessages(false);
	}
	...
	private ApiInfo getApiInfo() {
		return new ApiInfo(apiName, apiDescription, apiVersion, null, new Contact(contactName, null, contactEmail),
				null, null, Collections.emptyList());
	}

	/**
	 * Configures the Swagger UI: - Activates/Deactivates the "TRY IT OUT" button
	 * for Swagger UI
	 * 
	...
	public UiConfiguration uiConfig() {
		/*
		 * - Available methods are: "get", "head", "post", "put", "delete", "connect",
		 * "options", "trace", "patch" - Leave blank if you want to disable the 'TRY IT
		 * OUT' button for all methods
		 */
